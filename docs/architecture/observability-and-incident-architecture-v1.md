# IB Camp Observability And Incident Architecture v1
...
This document defines how IB Camp should detect, understand, and respond to production problems.
...
IB Camp should use one shared observability model with separate signal lanes and one explicit incident model.
...
IB Camp should treat observability as several related systems, not one giant event bucket.
...
IB Camp depends on multiple managed providers, so observability must include provider health, not only app health.
...
IB Camp should run production operations on a lightweight but explicit observability model:
