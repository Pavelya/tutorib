# IB Camp Data Subject Request Workflow v1
...
IB Camp already has a field-level retention and erasure map.
...
IB Camp should implement data subject requests as a **durable, reviewed, role-scoped workflow**.
...
A request to know what personal data IB Camp processes about the person.
...
IB Camp is minor-aware but should not support general self-service under-13 accounts for MVP.
...
- IB Camp controls its own references to external videos, not provider-side media owned by the tutor
...
- provider follow-up must be explicit where product data leaves IB Camp
...
IB Camp should handle privacy requests as a calm, explicit operational workflow.
