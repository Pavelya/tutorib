# IB Camp Database Observability And Maintenance v1
...
IB Camp now has a strong data-model and query-boundary architecture.
...
IB Camp should use a **lightweight but explicit database operations model**:
...
IB Camp has explicit query SLOs.
...
IB Camp must still know whether the domain operation succeeded.
...
For MVP, Supabase handles much of the database platform work, but IB Camp still needs to watch:
...
IB Camp should keep the database operations model simple, but not vague.
