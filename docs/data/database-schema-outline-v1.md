# IB Camp Database Schema Outline v1
...
This document defines the first canonical database shape for IB Camp.
...
IB Camp should use one normalized relational write model plus a small explicit projection layer.
...
IB Camp should reference them, not recreate them.
...
- IB Camp owns lesson scheduling and lesson state, not native video conferencing
...
IB Camp should use a relational schema that mirrors the approved shared object model:
